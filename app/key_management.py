...
def generate_certificate(username: str, private_key, public_key) -> x509.Certificate:
    subject = issuer = x509.Name([
        ...
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Secure Bookshelf"),
    ])
    ...
    cert = (
        ...
        .not_valid_after(now + datetime.timedelta(days=EXPIRY))
...
def add_member(aes_key: bytes, new_member_cert_pem: bytes) -> bytes:
    ...
    return wrap_group_key(aes_key, cert.public_key())


def remove_member(remaining_member_certs_pem: list) -> tuple:
    aes_key = generate_aes_key()
    new_group = {}
    for cert_pem in remaining_member_certs_pem:
        cert = deserialize_certificate(pem_data=cert_pem)
        username = get_username_from_cert(cert)
        new_group[username] = wrap_group_key(aes_key, cert.public_key())
